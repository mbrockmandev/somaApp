...
//
//  Using some Design Code modifiers
...
struct BackgroundStyle: ViewModifier {
  ...
  func body(content: Content) -> some View {
    content
      ...
      .shadow(color: Color(.systemGray).opacity(isLiteMode ? 0 : 0.3), radius: 20, x: 0, y: 10)
...
struct LargeSectionModifier: ViewModifier {
  func body(content: Content) -> some View {
    content
...
extension View {
  ...
  func backgroundStyle(cornerRadius: CGFloat = 20, opacity: Double = 0.2) -> some View {
  ...
  func largeSectionModifier() -> some View {
    modifier(LargeSectionModifier())
  }
